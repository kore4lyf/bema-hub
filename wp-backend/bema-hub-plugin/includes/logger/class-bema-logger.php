...
class Bema_Hub_Logger
{
    ...
    private static $default_config = [
        ...
        'log_level' => 'info'  // Changed from 'warning' to 'info' to capture more logs
    ];
    ...
    public function __construct($identifier, $config = [])
    {
        ...
        // Define log directory and file paths (consistent path)
        $this->log_dir = \WP_CONTENT_DIR . '/uploads/bema-crowdfunding-logger/' . $this->identifier;
        $this->log_file = $this->log_dir . '/' . $this->identifier . '.log';

        // Generate correlation ID for this logger instance
        $this->correlation_id = uniqid('Bema_Crowdfunding_', true);

        // Ensure the log directory exists and is secure.
        $this->ensure_log_directory();

        // Schedule log cleanup to run periodically, but only if not during activation
        $is_installing = defined('WP_INSTALLING') ? \WP_INSTALLING : false;
        if (!$is_installing) {
    ...
    {
        $this->correlation_id = uniqid('Crowdfunding_', true);
    ...
    {
        if (!\wp_next_scheduled('bema_crowdfunding_log_cleanup_' . $this->identifier)) {
            \wp_schedule_event(time(), 'daily', 'bema_crowdfunding_log_cleanup_' . $this->identifier);
        }
        
        \add_action('bema_crowdfunding_log_cleanup_' . $this->identifier, [$this, 'cleanup_old_logs']);
    }
